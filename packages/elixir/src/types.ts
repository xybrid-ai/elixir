...
export interface FallbackPolicy {
  ...
  timeoutMs?: number;
  /**
   * Extra unmarked statuses that trigger fallback, on top of gateway-marked
   * errors and `retryServerErrors`. Use this only for statuses known to be
   * generated by your gateway; an unmarked response may be the provider's own
   * answer, and replaying it upstream could duplicate the request.
   * @default []
   */
  retryStatuses?: number[];
  /**
   * Fall back on gateway-marked server errors and unmistakable edge-proxy
   * errors (Cloudflare 520–530), unless the status is explicitly listed in
   * `retryStatuses`.
   * @default true
   */
